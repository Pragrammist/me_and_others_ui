...
// build/dev/javascript/front_ui/message_types.mjs
var UserClickThoughtPanel = class extends CustomType {
  constructor(msg_type) {
    super();
    this.msg_type = msg_type;
  }
};
var ThoughtList = class extends CustomType {
};
var AddThought = class extends CustomType {
};
var SearchByAttachment = class extends CustomType {
};
var SearchByThoughtContent = class extends CustomType {
};

// build/dev/javascript/front_ui/model_types.mjs
var AccountPageState = class extends CustomType {
  constructor(header_state, body_state) {
    super();
    this.header_state = header_state;
    this.body_state = body_state;
  }
};
var ShowThoughtSearchAttachmentDetails = class extends CustomType {
};
var ShowThoughtSearchInput = class extends CustomType {
};
var ShowEmptyHeader = class extends CustomType {
};
var ShowAddThoughtInput = class extends CustomType {
};
var ShowThoughtList = class extends CustomType {
};
...
// build/dev/javascript/front_ui/icons.mjs
function icon_template(icon_name, class_name_size) {
  return div(
    toList([class$(class_name_size)]),
    toList([
      img(
        toList([class$(class_name_size), src("/priv/static/icons/" + icon_name)])
      )
    ])
  );
}
function small(icon_name) {
  return icon_template(icon_name, "min-w-8 min-h-8 max-w-8 max-h-8");
}
function add_small() {
  return small("add_small.svg");
}
function account_small() {
  return small("accaount_small.svg");
}
function groups_small() {
  return icon_template("groups_small.svg", "min-w-16 max-w-16 min-h-8 max-h-8");
}
function message_small() {
  return small("message_small.svg");
}
function search_icon() {
  return small("search_small.svg");
}
function list_small() {
  return small("list_small.svg");
}
function attachment_small() {
  return small("attachment_small.svg");
}
function sticker_small() {
  return small("sticker_small.svg");
}
function smile_small() {
  return small("smile_small.svg");
}
function giff_small() {
  return small("giff_small.svg");
}
function enter_icon_small() {
  return small("enter_small.svg");
}

// build/dev/javascript/front_ui/view_components.mjs
function button_text_base(text2, click_type, icon) {
  return div(
    toList([
      class$("flex gap-4 items-center hover:cursor-pointer"),
      on_click(new UserClickThoughtPanel(click_type))
    ]),
    toList([
      icon(),
      span(toList([class$("font-ui-primary")]), toList([text(text2)]))
    ])
  );
}
function thoughts_main_header_buttons() {
  return div(
    toList([class$("flex gap-12 justify-end h-fit w-fit")]),
    toList([
      message_small(),
      groups_small(),
      account_small()
...
function add_thought_input() {
  return div(
    toList([class$("flex gap-8 bg-secondary p-8 rounded-lg")]),
    toList([
      attachment_small(),
      input(
        toList([class$("input-large w-158"), placeholder("\u0412\u0430\u0448\u0430 \u043C\u044B\u0441\u043B\u044C")])
      ),
      sticker_small(),
      smile_small(),
      giff_small(),
...
function account_body(msg) {
  let choose_body = (body_state) => {
    if (body_state instanceof ShowAddThoughtInput) {
      return add_thought_input();
    } else {
  ...
  let body_element = (() => {
    {
      let body_state = msg.body_state;
      return choose_body(body_state);
    }
  })();
  return div(toList([class$("")]), toList([body_element]));
}
function thoughts_list_button() {
  return button_text_base(
    "\u0412\u0441\u0435 \u043C\u044B\u0441\u043B\u0438",
    new ThoughtList(),
    list_small
  );
}
function add_thought_button() {
  return button_text_base(
    "\u041C\u044B\u0441\u043B\u044F",
    new AddThought(),
    add_small
...
function search_thougts_button() {
  return button_text_base(
    "\u0411\u043B\u0443\u0436\u0434\u0430\u043D\u0438\u0435 \u043F\u043E \u0440\u0430\u0437\u0443\u043C\u0443",
    new SearchByThoughtContent(),
    search_icon
  );
}
function search_by_attacment_button() {
  return button_text_base(
    "\u0412\u043B\u043E\u0436\u0435\u043D\u0438\u044F",
    new SearchByAttachment(),
    attachment_small
  );
}
function thoughts_panel() {
  return div(
    toList([class$("flex justify-space-between w-fit h-fit gap-8")]),
    toList([
...
function account_header() {
  return div(
    toList([class$("flex h-64 py-8 px-4 justify-between w-full")]),
    toList([thoughts_panel(), thoughts_main_header_buttons()])
  );
}

// build/dev/javascript/front_ui/front_ui.mjs
function init2(_) {
  let model = new AccountPageState(
    new ShowEmptyHeader(),
    new ShowThoughtList()
  );
  return [model, none()];
...
function update(model, msg) {
  let handle_thought_user_click = (msg_type) => {
    debug("ewq");
    if (msg_type instanceof AddThought && model instanceof AccountPageState) {
      let header_state = model.header_state;
      return new AccountPageState(
        header_state,
        new ShowAddThoughtInput()
      );
    } else if (msg_type instanceof SearchByAttachment && model instanceof AccountPageState) {
      let body_state = model.body_state;
      return new AccountPageState(
        new ShowThoughtSearchAttachmentDetails(),
        body_state
      );
    } else if (msg_type instanceof SearchByThoughtContent && model instanceof AccountPageState) {
      let body_state = model.body_state;
      return new AccountPageState(
        new ShowThoughtSearchInput(),
        body_state
      );
    } else {
      let header_state = model.header_state;
  ...
  let model$1 = (() => {
    {
      ...
      return handle_thought_user_click(msg_type);
    }
  })();
  return [model$1, none()];
}
function view(model) {
  return div(
    toList([class$("w-full flex justify-center bg-primary")]),
    toList([
      div(
        toList([class$("w-256 min-h-screen")]),
        toList([
          account_header(),
          account_body(model)
        ])
